...
def index(request):
    ...
    if request.method == 'POST':
        ...
        item_queryset = Item.objects.filter(title=listing_title)
        
        if item_queryset.exists():
            item_ = item_queryset.first()  # Use first to avoid MultipleObjectsReturned
            user_ = User.objects.filter(username=request.user.username).first()  # Assuming this is unique

            search_item = Watchlist.objects.filter(items=item_, user=user_)
            if not search_item.exists():  # Use exists() to check for non-empty queryset
                message = "Item added to watchlist"
                Watchlist(items=item_, user=user_).save()
            else:
                message = "Item has already been added to watchlist"
        else:
            message = "Item not found"  # Handle item not found case
    ...
    if items:
        ...
        for item in items:
            if item in items_with_bids:
                bid_amount = Bid.objects.filter(items=item).order_by('amount').last()
                bids.append(bid_amount.amount if bid_amount else item.starting_bid)
            else:
